...
def add_paragraphs_with_subsections(doc, text, main_heading):
    doc.add_heading(main_heading, 1)
    doc.add_paragraph("")  
    ...
    for line in lines:
        ...
        if stripped.startswith("### "):
            doc.add_heading(stripped[4:], 2)  #remove ### and make it subheading
        if stripped.startswith("##"):
            continue
        elif stripped:
            doc.add_paragraph(stripped)

def generate_docx(summary, introduction, objectives, methodology, results_section, conclusion): 
    doc = Document() 
    ...
    add_paragraphs_with_subsections(doc, results_section, "Results")
